...
export const GovernanceActivityTimeline = ({ activities = [], className = '' }) => {
  if (!activities.length) {
    return (
      <Card className={`p-4 text-center text-xs text-slate-400 ${className}`}>
        No corporate governance activity logged yet.
      </Card>
    );
  }
  ...
  return (
    <div className={`space-y-2.5 ${className}`}>
      {activities.map((act) => {
        const actorName = act.actor?.name || 'System / Board';
        const dateStr = new Date(act.timestamp || act.createdAt).toLocaleString();
          ...
          >
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-[11px] text-slate-400 font-mono">
                Actor: <span className="text-slate-300">{actorName}</span> • Event: <span className="text-purple-300">{act.eventType}</span>
              </p>
            </div>
          </div>
        );
...
export default GovernanceActivityTimeline;
