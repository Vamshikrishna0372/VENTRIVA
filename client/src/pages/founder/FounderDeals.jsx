...
import { GitPullRequest, ArrowRight, Loader2, Building2 } from 'lucide-react';
import { Card, CardHeader, CardBody } from '../../components/common/Card';
import { Button } from '../../components/common/Button';
import DealStatusBadge from '../../components/deals/DealStatusBadge';
import { getMyDeals } from '../../services/dealService';

export const FounderDeals = () => {
  ...
  const fetchDeals = async () => {
    ...
    try {
      ...
      console.error('Error fetching founder deals:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 space-y-2">
        <h1 className="text-2xl font-bold text-slate-100">Investment Deal Rooms</h1>
        <p className="text-sm text-slate-400">Review active investment deal proposals, term sheets, negotiation history, and closing checklists.</p>
      </div>
        ...
        <Card className="text-center py-12">
          <CardBody className="space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-md mx-auto">
              When an investor advances your startup in their pipeline to a formal deal, active deal rooms will appear here.
            </p>
          </CardBody>
        </Card>
      ) : (
            ...
            <Card key={deal._id} className="hover:border-slate-700 transition-all">
              <CardBody className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Investor: {deal.investor?.name || deal.investor?.organization || `${deal.investor?.firstName || ''} ${deal.investor?.lastName || ''}`.trim() || 'Investor'}</p>
                    </div>
                  </div>
                ...
                <div className="flex justify-end pt-2 border-t border-slate-800">
                  <Link to={`/founder/deals/${deal._id}`}>
