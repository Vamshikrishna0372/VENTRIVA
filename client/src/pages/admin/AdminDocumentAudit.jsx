...
export const AdminDocumentAudit = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      console.error('Error fetching document access logs:', err);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Document Access Audit Logs</h1>
            <p className="text-sm text-slate-400">Security audit history of data room document views, downloads, and denied access attempts.</p>
          </div>
          <Badge variant="brand">{pagination.total} Document Events Logged</Badge>
        </div>

        {/* Filters */}
        <div className="pt-2 border-t border-slate-800 max-w-md">
      ...
      {isLoading ? (
        <div className="min-h-[300px] flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-slate-400 font-mono">Loading Document Access Trail...</p>
        </div>
      ) : logs.length === 0 ? (
        <Card className="text-center py-16 px-4 space-y-4">
          <History className="w-12 h-12 text-slate-500 mx-auto" />
          <div className="max-w-md mx-auto space-y-1">
            <h3 className="text-lg font-bold text-slate-100">No Document Access Logs Found</h3>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {logs.map((log) => (
                <tr key={log._id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="p-4">
                    <Badge variant={log.action === 'ACCESS_DENIED' ? 'rose' : 'emerald'} size="xs">
                      {log.action}
                    </Badge>
                  </td>
                  <td className="p-4 font-sans text-slate-200">{log.description}</td>
                  <td className="p-4">
                    <div className="font-bold text-slate-100">{log.admin?.name || 'User'}</div>
